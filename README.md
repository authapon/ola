...
- **ข้อความที่ตอบกลับผู้ใช้จริงๆ ไม่มีแถบวันเวลานี้ปนอยู่เลย** — สิ่งที่ส่งกลับไปหาผู้ใช้คือคำตอบดิบของโมเดิลตรงๆ ไม่เคยผ่านการแปะแถบวันเวลา/ชื่อผู้พูดเลย (แถบนี้มีไว้ให้ "โมเดิลอ่าน" เป็น input เท่านั้น ไม่ใช่สิ่งที่ควรปรากฏใน output) — **แต่กฎใน system prompt เพียงอย่างเดียวไม่พอในทางปฏิบัติ**: พบจากการใช้งานจริงว่าโมเดิลบางตัวยังคงพิมพ์แถบนี้ขึ้นเองในคำตอบแม้จะมีกฎห้ามไว้แล้ว และยิ่งแย่ลงเรื่อยๆ ในบทสนทนาเดียวกัน (พอโมเดิลพิมพ์แถบนี้ครั้งแรก คำตอบนั้นถูกบันทึกเก็บไว้ พอถูก replay กลับมาเป็นประวัติในเทิร์นถัดไปก็จะมีแถบใหม่ห่อซ้อนอยู่ข้างนอกอีกชั้น ทำให้โมเดิลยิ่งเข้าใจว่า "คำตอบของตัวเองต้องขึ้นต้นแบบนี้" และเลียนแบบต่อไปเรื่อยๆ แม้แต่ข้อความที่ไม่เกี่ยวกับวันที่เลยก็ตาม) — **แก้ด้วยการตัดแถบนี้ออกด้วยโค้ดโดยตรง** (`stripEchoedTimestampBracket`, ทำงานที่จุดเดียวคือ `runChatToolLoop` ครอบคลุมทั้ง 4 บอท) ก่อนส่งคำตอบให้ผู้ใช้และก่อนบันทึกลง context เสมอ ตัดวงจรการเลียนแบบซ้ำๆ นี้ได้อย่างแน่นอน ไม่ต้องพึ่งว่าโมเดิลจะทำตามกฎหรือไม่
- **log ไฟล์ (`.log`) ก็มีวันเวลากำกับทุกบรรทัดเช่นกัน** (รูปแบบ `[2026-08-16 14:32:05]` แบบมาตรฐาน เรียงลำดับ/ค้นหาง่ายสำหรับผู้ดูแลระบบ ต่างจากรูปแบบภาษาไทยที่ใช้ฝังใน context ซึ่งเน้นให้อ่านง่ายสำหรับคนไทย/โมเดิล) เพิ่มเข้าไปที่จุดเดียว (`chatBotCore.logf`) ครอบคลุมทุกบรรทัด log ของทั้ง 4 บอทโดยอัตโนมัติ ไม่ต้องแก้ทีละจุด
...
- **แนบรูปให้อัตโนมัติสำหรับกรณีที่พบบ่อยที่สุด (ไม่ต้องพึ่งให้โมเดิลเรียก `read_pic` เอง):** ถ้าเทิร์นก่อนหน้าในแชท (เทิร์นล่าสุดก่อนคำถามนี้ทันที) เป็นรูปที่เพิ่งถูกบันทึกไว้ (มี tag `[แนบรูปภาพ: pics/...]`) และข้อความที่ถูก mention ตอนนี้ไม่ได้แนบรูปมาเอง ระบบจะ**โหลดรูปนั้นแนบให้อัตโนมัติในทันที** โดยไม่ต้องรอให้โมเดิลตัดสินใจเรียก `read_pic` เลย — เพิ่มเข้ามาหลังพบจริงจากการใช้งานจริงว่าบางโมเดิล (โดยเฉพาะที่ต้องพึ่ง cloud routing) **ไม่เรียก `read_pic` เลยแม้จะมีกฎบังคับใน system prompt แล้วก็ตาม** แล้วตอบแบบเดา/มั่วขึ้นมาทั้งดุ้นแทน (ยืนยันจาก log จริงที่ไม่มี `read_pic` tool call เลยสักครั้ง) กลไกนี้ครอบคลุมเฉพาะ**เทิร์นก่อนหน้าเทิร์นเดียว** เท่านั้น (ตรงกับ pattern ที่พบบ่อยที่สุด: โพสต์รูปเฉยๆ แล้วถามทันทีว่า "รูปอะไร") ถ้าต้องการดูรูปที่ส่งไปนานกว่านั้น โมเดิลยังต้องเรียก `read_pic` เองตามปกติอยู่ (มีข้อความกำกับให้ตอบตามรูปจริงเช่นเดียวกับตอน `read_pic`)
- **ข้อควรรู้เรื่องความแม่นยำ:** การอธิบายรูปที่เรียกผ่าน `read_pic` (ไม่ใช่กรณีแนบอัตโนมัติข้างต้น) อาจแม่นยำน้อยกว่าตอนแนบรูปให้บอทดูตรงๆ ในเทิร์นเดียวกัน เพราะมี tool call/tool result คั่นกลางระหว่างคำถามจริงกับรูปที่แนบมา ทำให้โมเดิล (โดยเฉพาะโมเดิลขนาดเล็กหรือบางเวอร์ชันที่ต้องพึ่ง cloud routing) มีโอกาส "หลุดโฟกัส" ได้ง่ายกว่า — ระบบมีข้อความกำกับชัดเจนที่ท้ายเทิร์น follow-up ย้ำให้โมเดิลตอบตามสิ่งที่เห็นจริงในรูปเท่านั้น ห้ามเดา, compaction ถูกสั่งให้เก็บ path ของรูปไว้แบบคำต่อคำเสมอ (ไม่ถอดความทิ้ง), และมีกฎบังคับให้โมเดิลต้องเรียก `read_pic` ก่อนตอบเสมอเมื่อคำถามพูดถึงรูปที่ไม่ได้แนบมาในข้อความปัจจุบัน แต่สุดท้ายความแม่นยำยังขึ้นกับว่าโมเดิลที่ใช้ทำตามคำสั่งได้ดีแค่ไหนด้วย
